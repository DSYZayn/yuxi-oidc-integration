...
│       │   └── auth_router_patch.md       # auth_router.py 补丁说明
│       └── utils/
...
在 `backend/server/routers/auth_router.py` 中添加以下导入：
...
5. 考虑启用 OIDC Provider 的 PKCE 支持（当前实现暂未支持，可后续添加）
